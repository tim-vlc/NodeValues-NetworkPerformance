...
def train(model,test_num,samples,input_dim,output_dim,sampling_method,max_epochs,activationFunction,train_size,test_size,state):
    ...
    for e in range(max_epochs):
        if e >= max_epochs * 0.90:
            model.collect = True
        optimizer.zero_grad()
        output = model.forward(x_train.float(), activationFunction)
        loss = loss_func(output, y_train.float().unsqueeze(1))
        loss.backward()
        optimizer.step()

    # train_nv = np.array(reformat_nv(model.node_values,train_size,max_epochs,model.depth))
    train_nv = model.node_values
    training_error = loss.item()
    model.node_values = []

    # output_test = model(x_test.float(), activationFunction)
    # generalization_error = loss_func(output_test, y_test.float().unsqueeze(1)).item()
    with torch.no_grad():
        output_test = model(x_test.float(), activationFunction)
        generalization_error = loss_func(output_test, y_test.float().unsqueeze(1)).item()

        gensample_err = []
        for i, res in enumerate(output_test):
            loss = loss_func(res, y_test.float().unsqueeze(1)[i])
            gensample_err.append(loss.item())

    # test_nv = np.array(reformat_nv(model.node_values,test_size,1,model.depth))
    # model.node_values = []
    test_nv = model.node_values
    model.node_values = []
    ...
    store_data([num_out,list_dist_nv,mean_dist,output_test,gensample_err], ["num_out","list_dist_nv","mean_dist","output_test", "gensample_err"], folder_name)
