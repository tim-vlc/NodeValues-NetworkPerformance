...
def dist_stdv(mean_nv, stdv_nv, test_nv):
    ...
    list_dist_nv = []
    #i = 0
    for sample_nv in test_nv:
        dist_nv = []
        #j = 0
        for mean, stdv, nv in zip(mean_nv, stdv_nv, sample_nv):
            times = stdv
            num_stdv = 0
            #print(i, j)
            if not (mean*0.99 < nv < mean*1.01):
                while (nv > times + mean) or (nv < mean - times):
                    times += stdv
                    num_stdv += 1
            dist_nv.append(num_stdv)
            #j += 1
        #i += 1
        list_dist_nv.append(dist_nv)
    return list_dist_nv
...
def writer(test_num,sampling_method,tot_mean,tot_out,gen_err,train_err,width,depth):
    cwd = os.getcwd()
    file_path = cwd + '/data/' + '/results_' + sampling_method + '.txt'
